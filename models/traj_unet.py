...
class WideAndDeep(nn.Module):
    def __init__(self, embedding_dim=128, hidden_dim=256, mode=''):
        ...
        self.label_embedding = nn.Embedding(6, hidden_dim)
...
class WideAndDeep_old(nn.Module):
    def __init__(self, embedding_dim=128, hidden_dim=256, mode=''):
        super(WideAndDeep, self).__init__()
...
class Model(nn.Module):
    ...
    def __init__(self, config):
        ...
        ch, out_ch, ch_mult = 128, 2, tuple([1, 2, 2, 2])
        num_res_blocks = 2
        attn_resolutions = [16]
        dropout = 0.1
        in_channels = 2
        resolution = config.traj_len
        ...
        # ])
        if config.use_img or config.use_traj:
            self.temb_ch = self.temb_ch + 3 + 64
        else:
            self.temb_ch = self.temb_ch + 3 
    ...
    def forward(self, x, beta, context, img_feat=None, traj_feat=None):
        ...
        context = context.view(batch_size, 1, -1)   # (B, 1, F) [1024, 1, 128]
        temb = torch.cat([beta, torch.sin(beta), torch.cos(beta)], dim=-1)  # (B, 1, 3) [1024, 1, 3]
        
        temb = torch.cat([temb, context], dim=-1)    # (B, 1, F+3)
        if img_feat is not None:
            img_feat = img_feat.view(batch_size, 1, -1)
            temb = torch.cat([temb, img_feat], dim=-1)  # (B, 1, F+3+1000)
        if traj_feat is not None:
            traj_feat = traj_feat.view(batch_size, 1, -1)
            temb = torch.cat([temb, traj_feat], dim=-1)  # (B, 1, F+3+64)
        temb = temb.squeeze(1)
    
        x = x.permute(0,2,1)    # [1024, 2, 102]
        # downsampling
        hs = [self.conv_in(x)]  
        # hs[-1].shape,             temb.shape
        # torch.Size([10, 128, 200]) torch.Size([10, 512])
        # hs[-1] = hs[-1].permute(0,2,1)
...
class Guide_UNet(nn.Module):
    ...
    def forward(self, x, beta, context, img_feat=None, traj_feat=None):
        guide_emb = self.guide_emb(context)
        if img_feat is not None:
            img_feat = self.dim_converter(img_feat)
        cond_noise = self.unet(x, beta, guide_emb, img_feat, traj_feat)
        cond_noise = cond_noise.permute(0,2,1)
        return cond_noise
